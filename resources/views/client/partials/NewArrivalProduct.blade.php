@foreach ($categories as $catItem)
    <div class="container clearfix">
        ...
        <div class="row grid-6">
            @foreach ($catItem->products as $product)
                        ...
                        <div class="product-image">
                            ...
                            <a href="{{ route('client.showProductDetails', ['slug' => $product->slug]) }}">
                                ...
                                <div class="hoverbuttonbox">
                                    <div class="buttonaction" data-hover-animate="fadeIn" data-hover-speed="400">
                                        @guest
                                            <a href="javascript:void(0);"
                                                ...
                                                class="btn btn-dark"><i class="icon-heart3"></i>
                                                <span> ({{ $product->favorite_to_users->count() }})</span>
                                            </a>
                                        @else
                                            <a href="javascript:void(0);"
                                                onclick="document.getElementById('favorite-form-{{ $product->id }}').submit();"
                                                class="{{ !Auth::user()->favorite_product->where('pivot.product_id', $product->id)->count() == 0
                                                    ? 'favorite_posts'
                                                    : '' }}"><i
                                                    class="icon-heart3"></i><span class="text-dark">(<span
                                                        class="favorite_posts">{{ $product->favorite_to_users->count() }}</span>)</span>
                                            </a>

                                            <form id="favorite-form-{{ $product->id }}" method="POST"
                                                action="{{ route('client.favorite', $product->id) }}"
                                                style="display: none;">
                                                @csrf
                                            </form>
                                        @endguest
                        ...
                        <div class="product-desc">
                            ...
                            <div class="product-price font-primary">
                                @if ($product->product_price != $product->product_selling_price)
                                    <del class="mr-1">&#2547; {{ $product->product_price }}</del>
                                @endif <ins>&#2547; {{ $product->product_selling_price }}</ins>
                            </div>
